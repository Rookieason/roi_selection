...
# 		csi_path = resolve_csi_input_path(args.data_path, exp_name)
# 		print(f"Loading CSI: {csi_path}")
# 		CSI = np.load(csi_path)['csi']
...
# def resolve_csi_input_path(data_path, exp_name):
# 	csi_dir = Path(data_path) / "csi"
# 	candidates = [
# 		csi_dir / f"csi_{exp_name}.npz",
# 		csi_dir / f"{exp_name}.npz",
# 		csi_dir / f"{exp_name}_merged.npz",
# 	]
...
# 	matches = sorted(csi_dir.glob(f"*{exp_name}*.npz"))
# 	if len(matches) == 1:
# 		return matches[0]
# 	if len(matches) > 1:
# 		candidate_list = "\n".join(f"  - {path}" for path in matches)
# 		raise FileExistsError(
# 			f"Multiple CSI files match exp_name '{exp_name}'. Use a more specific exp_name.\n{candidate_list}"
# 		)

# 	candidate_list = "\n".join(f"  - {path}" for path in candidates)
# 	raise FileNotFoundError(
# 		f"No CSI file found for exp_name '{exp_name}'. Checked:\n{candidate_list}"
# 	)
        ...
        for heatmap_type in args.heatmap_type:
            ...
            if heatmap_type == "ToF-Doppler":
                ...
                if args.save_mat:
                    mat_path = Path(args.save_path) / "heatmap_result" / "mat" / exp_name / heatmap_type
                    mat_path.mkdir(parents=True, exist_ok=True)
                    output_file = mat_path / "smoothed_CSI_avg.mat"
                    idx = np.arange(spectrums.shape[0], dtype=np.int64)
                    spectrums = spectrums.astype(np.float64)
                    savemat(output_file, {"idx": idx, "spectrum": spectrums})
                    print(f"[OUT] Saved MAT: {output_file}")
